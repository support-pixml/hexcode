...
class ControllerJournal3Checkout extends \Journal3\Opencart\Controller {
	...
	public function index() {
		...
		$cart_products = $data['checkout_data']['products'];
		// echo '<pre>'; print_r($data['checkout_data']['totals']); echo '</pre>';
		$cart_total = 0;
		foreach($cart_products as $cart_product)
		{
			$cart_total += intval(str_replace(',', '', str_replace('₹', '', $cart_product['total']))); 
		}
		// echo $cart_total;die;
		for($i = 0; $i<count($data['checkout_data']['totals']); $i++)
		{
			if($data['checkout_data']['totals'][$i]['title'] == 'Sub-Total')
			{
				$data['checkout_data']['totals'][$i]['title'] = 'Cart Total';
				$data['checkout_data']['totals'][$i]['text'] = $this->currency->format($cart_total, $this->session->data['currency']);
			}
			if($data['checkout_data']['totals'][$i]['title'] == 'Total')
			{
				$data['checkout_data']['totals'][$i]['title'] = 'Total';
				$data['checkout_data']['totals'][$i]['text'] = '<strong class="color-success">'.$data['checkout_data']['totals'][$i]['text'].'</strong>';
			}
		}
	...
	public function save() {
		...
		{
			if($data['totals'][$i]['title'] == 'Sub-Total')
			{
				$data['totals'][$i]['title'] = 'Cart Total';
				$data['totals'][$i]['text'] = $this->currency->format($cart_total, $this->session->data['currency']);
			}
			// if($data['totals'][$i]['title'] == 'Total')
			// {
			// 	$data['totals'][$i]['title'] = 'Total';
			// 	$data['totals'][$i]['text'] = '<strong class="color-success">'.$data['totals'][$i]['text'].'</strong>';
			// }
		}
...
//					$error['total_mismatch'] = 'Total mismatch, page will reload.';
	...
	private function totals($totals) {
		...
		foreach ($totals as $total) {
			if($total['title'] == 'CGST 2.5%' || $total['title'] == 'SGST 2.5%' || $total['title'] == 'IGST 5%')
			continue;
			$result[] = array(
				'title' => $total['title'],
