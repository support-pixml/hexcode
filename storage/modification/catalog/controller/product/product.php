		...
		if ($product_info) {
            ...
            'image'                   => defined('JOURNAL3_ACTIVE') ? ($option_value['image'] ? $this->model_journal3_image->resize($option_value['image'], $this->journal3->settings->get('image_dimensions_options.width'), $this->journal3->settings->get('image_dimensions_options.height'), $this->journal3->settings->get('image_dimensions_options.resize')) : false) : $this->model_tool_image->resize($option_value['image'], 50, 50),
				...
				$data['options'][] = array(
					...
					'checkboxmaxchecks'    => $option['checkboxmaxchecks']
				);
			...
			{
				if($opt['type'] == 'checkbox' && $opt['checkboxmaxchecks'] != 0)
				{
					$js_script = 'var checkboxmaxchecks = '.$opt['checkboxmaxchecks'].';';
					$js_script .= 'var product_option_id = '.$opt['product_option_id'].';';
					$js_script .= '$(\'input[name="option['.$opt['product_option_id'].'][]"]\').change(function() {
						
						if($(\'input[name="option['.$opt['product_option_id'].'][]"]:checked\').length > checkboxmaxchecks)
						{ $(this).prop("checked", false);
						alert("You have already selected '.$opt['checkboxmaxchecks'].' '.$opt['name'].'."); }
					});';
				}
				else
				{
